        ...
        if matched_players and #matched_players > 0 then
            ...
            for _, player in ipairs(matched_players) do
                ...
                if player and players[player.id] == player and player.fd and player.gate_service then
                    ...
                    skynet.error(string.format("Matched player %d has no valid connection, skipping", player and player.id or -1))
    ...
    while true do
                ...
                for _, player in pairs(players) do
                    if player.room_id == room_id then
                        ...
                        if player.fd and player.gate_service then
                            ...
                            skynet.error(string.format("Player %d (%s) has no valid connection, removing from players table", player.id, player.name))
                            players[player.id] = nil
                        end
                    end
                end
        ...
        for _, player in ipairs(players_to_rematch) do
            ...
            if player.fd and player.gate_service and players[player.id] == player then
                ...
                skynet.error(string.format("Player %d (%s) lost connection, not re-adding to queue", player.id, player.name))
                players[player.id] = nil
            end
        end
    end
...
function CMD.create_player_on_handshake(fd, player_name, gate_service)
    ...
    if not gate_service then
        skynet.error(string.format("[match_loop] Invalid gate_service (%s) for player creation, rejecting", tostring(gate_service)))
        return nil
    end
...
function CMD.add_player_to_queue(player_id)
    ...
    if player then
        ...
        if player.fd and player.gate_service then
            ...
            skynet.error(string.format("[match_loop] Player %d (%s) has no valid connection (fd=%s, gate=%s), not adding to queue", 
...
function CMD.add_player(player_id, player_name, gate_service, fd)
    skynet.error(string.format("[match_loop] WARNING: CMD.add_player called but is disabled. id=%d, name=%s, gate=%s, fd=%d", 
...
function CMD.register_room_service(room_id_param, room_service)
...
function CMD.room_game_ended(room_id_param)
    local room = rooms[room_id_param]
    if room then
...
function CMD.broadcast_state(room_id, state)
    ...
    skynet.error(string.format("[match_loop] Broadcast state to room %d: sent to %d players, failed: %d", room_id, sent_count, #failed))
